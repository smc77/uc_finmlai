...
Each arrow changes what can be concluded. A fitted relationship is not yet a useful forecast,
a useful forecast is not yet a trading rule, and a trading rule is not yet a net
investment result.

A second distinction runs underneath that chain. A **contemporaneous**
regression relates variables measured in the same period and often supports
explanation or risk attribution. A **predictive** regression uses information
available by one time to forecast an outcome that occurs later. A forecast must
be formed **ex ante**, before its outcome is known, even though its accuracy can
only be evaluated **ex post**, after the outcome arrives.
...
5. **Changing the estimator changes the question.** OLS targets a conditional mean
...
7. **Order and scale are different kinds of accuracy.** Predictive R² cares about numerical
...
| **Contemporaneous relationship** | Relates inputs and an outcome measured in the same period; it does not by itself establish forecasting ability. |
| **Predictive relationship** | Uses information available by a forecast time to estimate an outcome that occurs afterward. |
| **Ex ante** | Viewed from before the outcome is known, using the information available at that time. |
| **Ex post** | Viewed after the outcome is known; forecasts are evaluated ex post but must be reconstructed from ex-ante information. |
...
| **BLUE** | The narrow Gauss–Markov result that, under stated conditions, OLS is the lowest-variance linear unbiased estimator. |
...
- state whether the primary question concerns magnitude, ordering, or a decision;
